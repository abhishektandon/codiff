...
class ComposeUNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1d = nn.Conv2d(in_channels=12, out_channels=3, kernel_size=1)
        self.conv1d.weight.requires_grad = True
    ...
    def forward(self, x, t, cond):
        ...
        if ('seg_mask' in self.conditions) and ('text' in self.conditions) and ('sketch' not in self.conditions):
            ...
            seg_mask_confidence_map = seg_mask_confidence_map.repeat(1, 3, 1, 1)
            text_confidence_map = text_confidence_map.repeat(1, 3, 1, 1)
            
            concat = torch.cat([seg_mask_unet_output, seg_mask_confidence_map, text_unet_output, text_confidence_map], dim=1)
            output = F.relu(self.conv1d(concat.contiguous()), inplace=True)

            # grid = vutils.make_grid(output, nrow=1, normalize=True, scale_each=True)

            if not os.path.exists('test_images'):
                os.makedirs('test_images')

            for i in range(8):
                vutils.save_image(output[i], f'test_images/out_exp{i}.png')
                vutils.save_image(text_confidence_map[i], f'test_images/text_out_exp{i}.png')
                vutils.save_image(seg_mask_confidence_map[i], f'test_images/seg_conf_exp{i}.png')

            # print(concat.shape) # [8, 12, 64, 64]
